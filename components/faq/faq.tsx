...
export default async function FAQ({ page }: { page: string }) {
  ...
  const features = featuresT.raw(`${page}.features`) as FeatureItem[];
  ...
  return (
    <section className="my-8 md:my-16 bg-card/50 backdrop-blur-sm p-8 md:p-12 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 border border-border/40">
      <h2 className="text-2xl md:text-3xl font-bold mb-6 md:mb-8 text-center bg-gradient-to-r from-primary to-primary/80 bg-clip-text text-transparent">{faqT(`${page}.title`)}</h2>
      ...
      <div className="w-full max-w-4xl mx-auto">
        <h3 className="text-xl md:text-2xl font-semibold mb-6 md:mb-8 text-foreground/90">Frequently Asked Questions</h3>
        ...
        <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8 mt-6">
              {features.map((subItem, subIndex) => (
                <div key={`feature-item-${subIndex}`} className="border border-border/40 rounded-xl p-6 md:p-8 shadow-sm hover:shadow-md transition-all duration-300 bg-background/50 backdrop-blur-sm">
