...
import { FileIcon, CopyIcon, DownloadIcon, UploadIcon, ZapIcon, Code2Icon, ImageIcon, UndoIcon, RedoIcon } from 'lucide-react';
...
export default function EditorPage() {
  ...
  // Initialize history with the initial SVG code
  useEffect(() => {
    setHistory([svgCode]);
    setHistoryIndex(0);
  }, []);
  ...
  const handlePrettify = () => {
    try {
      ...
      const formattedSvg = serializer.serializeToString(xmlDoc)
        .replace(/></g, '>\n<')
        .replace(/><\/svg>/g, '>\n</svg>');
      
      handleCodeChange(formattedSvg);
      
      toast({
        title: "SVG Prettified",
        description: "Your SVG code has been formatted",
      });
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to prettify SVG code",
        variant: "destructive",
      });
    }
  };
      ...
      <header className="border-b">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <Link href="/editor" className="text-sm font-medium text-blue-600">
              Editor
            </Link>
      ...
      <main className="flex-1 container mx-auto px-4 py-6">
        <div className="flex flex-col gap-6">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            <h1 className="text-2xl font-bold">SVG Editor</h1>
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" onClick={handlePrettify} className="ml-4">
                Prettify
              </Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-12 gap-6">
            <div className="col-span-1">
              <SvgToolbar onAddElement={(element) => handleCodeChange(svgCode + element)} />
            </div>
            
            <div className="col-span-11 grid grid-cols-1 lg:grid-cols-2 gap-6">
